...
import {getallpatietarray,filterfromallpatients} from "@/Componenets/Hospital/getallpatientarray"
...
if (user ) {
  ...
  let cont= await makecontract()
    ...
    const Allpatientsar = await   cont.getallpatientsinhospital(hospitaldetails.name)
...
if ( error) {
  ...
  return (
    <Box   textAlign={ "start"}    boxSizing={"border-box"}  height={"auto"}     width={"100%"}  >
...
<TextField  label="enter the username to search"   onChange={(e)=>setsearch(e.target.value)}  value={search}   fullWidth  sx={{width:"500px"}}  size="small" />
...
{patient.name}
  ...
  <Stack spacing={2} width={"100%"}  direction={"row"} display={"flex"}>
    <Button
          size="small"
          color="secondary"
          onClick={()=>{
