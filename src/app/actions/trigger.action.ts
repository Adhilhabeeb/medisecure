...
export default async function triggerWorkflow(
  ...
  patientname: string,
  userdetails: hh,
  docterreviewofpatient: string
) {
  console.log(docterreporst, "i sterporst");
  const workflow_run_id = await knock.workflows.trigger("notquick", {
    data: {
      message: `  ${userdetails.name}   sented  a patient  ${docterreviewofpatient &&
        `has some ${docterreviewofpatient}`} and   patient   name  is `,
      patientname,
      docteremail:userdetails.email
    },
